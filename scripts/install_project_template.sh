...
Side effect of --with-scripts / --refresh-scripts: appends TARGET/.gitignore entries for
memory/.backups/ and memory/.fmc-source (append-if-missing, announced on stdout) so
generated runtime artefacts never land in the fork's git.
...
if [ "$with_scripts" -eq 1 ] || [ "$refresh_scripts" -eq 1 ]; then
  ...
  # Fix 4 (0.1.24, cc_pas field-report 2026-07-07): runtime artefacts that the engine delivery
  # itself generates (backups from --refresh-scripts into .backups/, the .fmc-source marker) must
  # not end up in the fork's git — so the installer writes them into .gitignore next to the memory
  # dir (append-if-missing, idempotent). Mechanism > "mention it in the README and hope".
  ...
  if [ -L "$gi_file" ]; then
    ...
    echo "would ensure gitignore entries in $gi_file ($gi_base/.backups/, $gi_base/.fmc-source, $gi_base/_rejstrik.md, $gi_base/_hot.md, $gi_base/.recall-state/, $gi_base/.recall-hits.log)"
  else
    for gi_line in "$gi_base/.backups/" "$gi_base/.fmc-source" "$gi_base/_rejstrik.md" "$gi_base/_hot.md" "$gi_base/.recall-state/" "$gi_base/.recall-hits.log"; do
