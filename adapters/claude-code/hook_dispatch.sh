...
case "$event" in
  ...
  user-prompt-submit)
    ...
    # in step with the Codex adapter (same reflex, host-specific plan tool). It holds ONE cumulative
    # session to-do ledger alive across pivots and pins the skill-use reflex to every step.
    printf '%s\n' \
      'FMC reflex — multi-step work: keep ONE cumulative visible to-do ledger (TodoWrite) for the WHOLE session — on a new direction/pivot MERGE into the same list, keep done items checked, NEVER replace the history with a fresh partial plan; a finished phase does not close the session ledger. Before each non-trivial step: does a skill cover it? -> find and use it.'
    ;;
